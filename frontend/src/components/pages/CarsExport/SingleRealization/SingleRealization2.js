...
import style from "./SingleRealization2.module.scss";
          ...
          <div className={style.gallery}>
            {restImgJsonParse.map((item, index) => (
              <motion.div
                ...
                onClick={() =>
                  handleImageClick(`${IMGS_URL}/export/cars/${item}`)
                }
              >
                <img src={`${IMGS_URL}/export/cars/${item}`} alt={`${index}`} />
              </motion.div>
            ))}
          </div>
        </div>
...
export default SingleRealization2;
